...
def main():
    ...
    new_tee_times_by_course: Dict[str, Any] = {}
    t0: time = time.time()

    for golf_course in golf_courses.values():
        print(f'{golf_course.name}')
        new_tee_times: Dict[str, Any] = golf_course.get_new_tee_times(weekday_cut_time, weekend_cut_time, tipper_config['min_spots'])
        if new_tee_times:
